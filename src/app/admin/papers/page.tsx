...
import { Search, Filter, FileText, User, ChevronRight } from 'lucide-react';
import { papersApi, usersApi } from '@/lib/api';
import { Paper, PaperStatus, User as UserType } from '@/types';
...
export default function AdminPapersPage() {
  ...
  const updateStatus = async (paperId: number, status: PaperStatus) => {
    try {
      await papersApi.adminUpdate(paperId, { status });
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-navy-500 text-sm font-sans">Manage all paper submissions and review assignments.</p>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-parchment-50 border-b border-parchment-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-navy-500 uppercase tracking-wider">Type</th>
            ...
            <tbody className="divide-y divide-parchment-100">
              {loading ? (
                ...
                <tr><td colSpan={7} className="text-center py-16 text-navy-400 font-sans">
                ...
                <tr key={paper.id} className="hover:bg-parchment-50 transition-colors">
                  ...
                  <td className="px-4 py-4">
                    <span className={`badge ${paper.paper_type === 'conference' ? 'bg-navy-100 text-navy-700' : 'bg-gold-100 text-gold-700'}`}>
                      {paper.paper_type}
                    </span>
                  </td>
                  <td className="px-4 py-4">
                    <select
                      value={paper.status}
                      onChange={e => updateStatus(paper.id, e.target.value as PaperStatus)}
                      className={`badge cursor-pointer border-0 ${getStatusColor(paper.status)} appearance-none pr-1`}
                    >
                      {STATUSES.map(s => <option key={s} value={s}>{getStatusLabel(s)}</option>)}
                    </select>
                  </td>
                  ...
                  <td className="px-4 py-4">
                    <Link href={`/admin/papers/${paper.id}`} className="text-gold-600 hover:text-gold-700">
