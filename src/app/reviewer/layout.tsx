...
export default function ReviewerLayout({ children }: { children: React.ReactNode }) {
  ...
  // The bottom tab strip only fits the reviewer's own three destinations, so
  // getting back to "/" or the personal dashboard needs this drawer too,
  // opened from the mobile top bar - same pattern as admin and chief editor.
    ...
    <div className="min-h-screen flex bg-navy-950">
      ...
      <header className="md:hidden fixed top-0 inset-x-0 z-30 h-14 bg-navy-900 border-b border-navy-800 px-2 flex items-center gap-1">
        <button
          ...
          aria-controls="reviewer-sidebar"
          className="p-3 rounded-xl text-parchment-100 hover:bg-navy-800 transition-colors"
        >
          <Menu className="w-5 h-5" />
        </button>
        <Link href="/reviewer" className="font-serif text-parchment-100 truncate">
          Prudent<span className="text-gold-400"> Journals</span>
        </Link>
        <div className="ml-auto flex items-center gap-1">
          <NotificationBell />
          <span className="mr-1 text-xs font-mono text-navy-400 uppercase tracking-widest truncate">Reviewer</span>
        </div>
      </header>
      ...
      {/* Off-canvas sidebar, below md - reaches "/" and "/dashboard" */}
      <aside
        id="reviewer-sidebar"
        className={cn(
